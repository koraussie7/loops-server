<template>
  <div class="dtube-content">
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Hot Videos</h1>
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
        What is buzzing on DTube right now
      </p>
    </div>
...
async function loadVideos() {
  ...
  error.value = null
  try {
    const results = await fetchHot({ limit: 50 })
    videos.value = results
    hasMore.value = results.length === 50
  } catch (e) {
    error.value = "Failed to load hot videos. Please try again."
    console.error("DTube hot error:", e)
  } finally {
    loading.value = false
  }
}
