<template>
  <div class="dtube-content">
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">New Videos</h1>
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
        Latest DTube uploads from across the Hive blockchain
      </p>
    </div>
...
async function loadVideos() {
  ...
  error.value = null
  try {
    const results = await fetchNew({ limit: 50 })
    videos.value = results
    hasMore.value = results.length === 50
  } catch (e) {
    error.value = "Failed to load new videos. Please try again."
    console.error("DTube new error:", e)
  } finally {
    loading.value = false
  }
}
